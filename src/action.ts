...
import { formatSummaryMarkdown } from './summary.js'
...
export async function run(): Promise<void> {
  ...
  const llmProvider = (getInputValue(
    'llm_provider',
    process.env.LLM_PROVIDER ?? 'openrouter',
    'LLM_PROVIDER',
  ) ?? 'openrouter') as import('./types.js').LLMProvider
  const llmApiUrl =
    getInputValue(
      'llm_api_url',
      process.env.LLM_API_URL ?? 'https://openrouter.ai/api/v1',
      'LLM_API_URL',
    ) ?? 'https://openrouter.ai/api/v1'
  const llmApiKey = getInputValue('llm_api_key', process.env.LLM_API_KEY ?? '', 'LLM_API_KEY')
  const llmModel =
    getInputValue('llm_model', process.env.LLM_MODEL ?? 'openrouter/free', 'LLM_MODEL') ??
    'openrouter/free'
  const reviewMode = (getInputValue(
    'review_mode',
    process.env.REVIEW_MODE ?? 'both',
    'REVIEW_MODE',
  ) ?? 'both') as import('./types.js').ReviewMode
  const toneMode = (getInputValue('tone_mode', process.env.TONE_MODE ?? 'balanced', 'TONE_MODE') ??
    'balanced') as import('./types.js').ToneMode
  const reviewLens = normalizeReviewLens(
    getInputValue('review_lens', process.env.REVIEW_LENS ?? 'default', 'REVIEW_LENS') ?? 'default',
  )
  const debiasedMode = parseBoolean(
    getInputValue('debiased_mode', process.env.DEBIASED_MODE ?? 'false', 'DEBIASED_MODE') ??
      'false',
  )
  ...
  const sarifPath = getInputValue('sarif_path', 'openrabbit.sarif')
  await mkdir(dirname(sarifPath), { recursive: true })
  await writeFile(sarifPath, buildSarif(response))
